...
public class CambioConverter {
    ...
    private static final String[] LIBRARY_SEARCH_PATHS = {
        System.getProperty("user.home") + "/GitHub/osh-oakridge-buildnode/dist/nativelibs",
        System.getProperty("user.home") + "/GitHub/cambio/SpecUtils/build_java",
        "/usr/local/lib",
        "/usr/lib"
    };
    ...
    private static synchronized void loadNativeLibrary() {
        ...
        // First try the standard way (respects java.library.path)
        try {
        ...
        // Try explicit paths
        for (String searchPath : LIBRARY_SEARCH_PATHS) {
            File libDir = new File(searchPath);
            if (!libDir.exists()) {
                continue;
            }

            // Try to load both required libraries
            File specUtilsLib = findLibrary(libDir, "SpecUtils");
            File jniLib = findLibrary(libDir, "SpecUtilsJni");

            if (jniLib != null) {
                try {
                    // Load SpecUtils first if available (JNI depends on it)
                    if (specUtilsLib != null) {
                        System.load(specUtilsLib.getAbsolutePath());
                    }
        ...
        throw new RuntimeException(
            "Failed to load SpecUtilsJni native library. " +
            "Set -Djava.library.path to the directory containing libSpecUtilsJni and libSpecUtils, " +
            "or place them in one of: " + String.join(", ", LIBRARY_SEARCH_PATHS)
        );
    }

    private static File findLibrary(File dir, String libName) {
        String os = System.getProperty("os.name").toLowerCase();
        String[] extensions;
        if (os.contains("mac")) {
            extensions = new String[]{".jnilib", ".dylib"};
        ...
        for (String ext : extensions) {
            File lib = new File(dir, "lib" + libName + ext);
            if (lib.exists()) {
                return lib;
            }
        }
