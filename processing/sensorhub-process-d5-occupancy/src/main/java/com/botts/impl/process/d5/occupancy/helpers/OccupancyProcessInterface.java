...
package com.botts.impl.process.d5.occupancy.helpers;
...
public class OccupancyProcessInterface extends OccupancyOutput<IDataProducer> {
     ...
     * datablocks as {@link OccupancyExtended} so the D5-specific
     ...
     * category string reported by the D5 driver.
     *
    ...
    public OccupancyProcessInterface(IProcessModule<?> parentProcess, AbstractSWEIdentifiable outputDescriptor, DataEncoding encoding) throws ProcessingException
    {
        ...
        if (outputDescriptor != null) {
            ...
            try {
                if (parentProcess instanceof D5OccupancyProcessModule d5Module) {
                    DataComponent execOutput = d5Module.wrapperProcess.getOutputComponent(outputDef.getName());
                    d5Module.wrapperProcess.connect(execOutput, outputQueue);
                }
     ...
     * therefore does not match the datastream schema registered for the D5
     * occupancy output.
     */
    @Override
